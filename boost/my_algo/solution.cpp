...
int singleNonDuplicate_On_simple_search(std::vector<int>& nums) {
	if (nums.size() == 1)
	{
		return nums[0];
	}
...
int singleNonDuplicate_On_XOR(std::vector<int>& nums) {
	int result = 0;
	for(auto cur: nums)
	{
		result ^= cur;
...
int singleNonDuplicate_Ologn_binary_search(std::vector<int>& nums) {
	int first = 0;
