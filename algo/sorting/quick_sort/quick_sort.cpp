...
constexpr std::ranges::borrowed_iterator_t<Range> quick_sort(Range&& rng, Comp comp = {}, Proj proj = {})
{
    if (std::ranges::size(rng) > 1) [[likely]] {
        ...
        while (!s.empty()) {
            ...
            auto mid = std::partition(first + 1, last, [=](auto const &v){ return comp(proj(v), proj(*first)); }) - 1;
