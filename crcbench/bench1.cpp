...
constexpr unsigned long long operator"" _KB(unsigned long long x) noexcept {
  return x << 10;
}
constexpr unsigned long long operator"" _MB(unsigned long long x) noexcept {
  return x << 20;
}
constexpr unsigned long long operator"" _GB(unsigned long long x) noexcept {
  return x << 30;
}
constexpr unsigned long long operator"" _TB(unsigned long long x) noexcept {
  return x << 40;
}
constexpr unsigned long long operator"" _PB(unsigned long long x) noexcept {
  return x << 50;
}
...
BENCHMARK(BM_CRC32CSoftware)
    ...
    ->Range(1_MB, 1_GB)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_BoostCRC32Optimal)
    ->RangeMultiplier(2)
    ->Range(1_MB, 1_GB)
    ->Unit(benchmark::kMillisecond);
